...
import 'ChecklistListView.dart';
import 'ChecklistItemView.dart';
export 'ChecklistItemView.dart';

typedef void OnDropChecklist(
    int oldListIndex, int listIndex, ChecklistViewState state);
typedef void OnTapChecklist(int listIndex, ChecklistViewState state);
typedef void OnStartDragChecklist(int listIndex, ChecklistViewState state);
...
class ChecklistView extends StatefulWidget {
  ...
  ChecklistView(
      ...
      this.index,
      this.isOpen,
      this.onDropChecklist,
      this.onTapChecklist,
      this.onStartDragChecklist,
  ...
  void onDropList(int listIndex) {
    widget.checklistState.setState(() {
      if (widget.onDropChecklist != null) {
        widget.onDropChecklist(widget.checklistState.oldListIndex,
            widget.checklistState.draggedListIndex, this);
      }
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.items != null) {
      for (int i = 0; i < widget.items.length; i++) {
        if (widget.items[i].checklist == null ||
            widget.items[i].index != i ||
            widget.items[i].checklist.widget.index != widget.index ||
            widget.items[i].checklist == this) {
        ...
        if (widget.checklistState.draggedItemIndex == i &&
          ...
          widgets.add(AnimatedCrossFade(
            ...
            crossFadeState: (widget.isOpen == null || widget.isOpen)
                ? CrossFadeState.showFirst
                : CrossFadeState.showSecond,
            firstChild: widget.items[i],
